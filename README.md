...
- [x] **Export / print the blueprint** — click "Download DXF (CNC)" to export the finished layout as a `.dxf` file. The DXF contains three layers: `WALL` (polygon outline, green), `TILES` (all tile rectangles, white), and `CUT_LINES` (cut lines across edge tiles where the wall boundary crosses, red). Open in any CAD package (FreeCAD, LibreCAD, Fusion 360) or send directly to a CNC tile cutter.
